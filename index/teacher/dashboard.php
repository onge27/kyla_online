...
if ($hasTeacherId) {
    ...
    $student_count = $conn->query("SELECT COUNT(DISTINCT sa.student_id) as count FROM student_answers sa JOIN exams e ON sa.question_id IN (SELECT id FROM questions WHERE exam_id = e.id) WHERE e.teacher_id = {$_SESSION['user_id']}")->fetch_assoc()['count'];
